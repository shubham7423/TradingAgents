...
def build_aggressive_prompt(state: Mapping[str, Any], *, output_language: str) -> str:
    """Build the Aggressive Risk Analyst prompt without invoking the model."""
...
def apply_aggressive_output(state: Mapping[str, Any], output: str) -> dict[str, Any]:
    risk_debate_state = state["risk_debate_state"]
    argument = f"Aggressive Analyst: {output}"
    return {
        "risk_debate_state": {
            "history": risk_debate_state.get("history", "") + "\n" + argument,
...
def create_aggressive_debator(llm):
    def aggressive_node(state) -> dict:
        ...
        response = llm.invoke(build_aggressive_prompt(state, output_language=language))
        return apply_aggressive_output(state, response.content)
